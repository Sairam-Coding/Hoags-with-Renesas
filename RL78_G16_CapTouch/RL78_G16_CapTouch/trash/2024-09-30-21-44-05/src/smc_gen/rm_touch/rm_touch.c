...
#include "Config_UART3.h"
 #elif (TOUCH_CFG_UART_NUMBER == 0xA0)
#include "Config_UARTA0.h"
 #elif (TOUCH_CFG_UART_NUMBER == 0xA1)
...
/* UART code generator function macro */
 #if (TOUCH_CFG_UART_NUMBER == 0)
  #define TOUCH_UART_START    R_Config_UART0_Start
  #define TOUCH_UART_STOP     R_Config_UART0_Stop
  #define TOUCH_UART_SEND     R_Config_UART0_Send
  #define TOUCH_UART_RECEIVE  R_Config_UART0_Receive
 #elif (TOUCH_CFG_UART_NUMBER == 1)
  #define TOUCH_UART_START    R_Config_UART1_Start
  #define TOUCH_UART_STOP     R_Config_UART1_Stop
  #define TOUCH_UART_SEND     R_Config_UART1_Send
  #define TOUCH_UART_RECEIVE  R_Config_UART1_Receive
 #elif (TOUCH_CFG_UART_NUMBER == 2)
  #define TOUCH_UART_START    R_Config_UART2_Start
  #define TOUCH_UART_STOP     R_Config_UART2_Stop
  #define TOUCH_UART_SEND     R_Config_UART2_Send
  #define TOUCH_UART_RECEIVE  R_Config_UART2_Receive
 #elif (TOUCH_CFG_UART_NUMBER == 3)
  #define TOUCH_UART_START    R_Config_UART3_Start
  #define TOUCH_UART_STOP     R_Config_UART3_Stop
  #define TOUCH_UART_SEND     R_Config_UART3_Send
  #define TOUCH_UART_RECEIVE  R_Config_UART3_Receive
 #elif (TOUCH_CFG_UART_NUMBER == 0xA0)
  #define TOUCH_UART_START    R_Config_UARTA0_Start
  #define TOUCH_UART_STOP     R_Config_UARTA0_Stop
  #define TOUCH_UART_SEND     R_Config_UARTA0_Send
  #define TOUCH_UART_RECEIVE  R_Config_UARTA0_Receive
 #elif (TOUCH_CFG_UART_NUMBER == 0xA1)
  #define TOUCH_UART_START    R_Config_UARTA1_Start
  #define TOUCH_UART_STOP     R_Config_UARTA1_Stop
  #define TOUCH_UART_SEND     R_Config_UARTA1_Send
  #define TOUCH_UART_RECEIVE  R_Config_UARTA1_Receive
 #endif
#endif
...
#if ((TOUCH_CFG_MONITOR_ENABLE && (TOUCH_CFG_UART_MONITOR_SUPPORT == 1)) || (TOUCH_CFG_UART_TUNING_SUPPORT == 1))
void touch_uart_callback(uint16_t event);
#endif
...
fsp_err_t RM_TOUCH_Open (touch_ctrl_t * const p_ctrl, touch_cfg_t const * const p_cfg)
{
    ...
    if (0 == g_touch_uart_open_bitmap)
    {
        ...
        TOUCH_UART_RECEIVE(&g_touch_uart_rx_buf[0], g_touch_uart_rx_num);
        g_touch_uart_monitor_num   = TOUCH_UART_MONITOR_NONE;
    }

    g_touch_uart_open_bitmap |= (uint32_t) ((uint32_t)1 << num);
 ...
 * @retval FSP_ERR_ASSERTION        Null pointer passed as a parameter.
 * @retval FSP_ERR_NOT_OPEN         Module is not open.
 * @retval FSP_ERR_INVALID_HW_CONDITION     Accuracy of data is not guaranteed.
                            ...
                            uint16_t           * p_wheel_position)
{
    fsp_err_t               err             = FSP_SUCCESS;
    touch_instance_ctrl_t * p_instance_ctrl = (touch_instance_ctrl_t *) p_ctrl;
    static uint16_t         data[CTSU_CFG_NUM_SELF_ELEMENTS + (CTSU_CFG_NUM_MUTUAL_ELEMENTS * 2)];
#if ((TOUCH_CFG_NUM_BUTTONS != 0) || (TOUCH_CFG_NUM_SLIDERS != 0) || (TOUCH_CFG_NUM_WHEELS != 0))
    ...
    *p_button_status = p_instance_ctrl->binfo.status;
    ...
    if (!g_touch_uart_transmit_flag && (p_instance_ctrl->p_touch_cfg->number == g_touch_uart_monitor_num))
    {
        ...
        TOUCH_UART_SEND((uint8_t *) g_touch_monitor_buf, index);
    }
 #endif
#endif
                               ...
                               uint8_t            * p_pad_num_touch)
{
    ...
    *(p_instance_ctrl->pinfo.p_tx_coordinate) = TOUCH_OFF_VALUE;
    ...
    for (i = 0; i < element_num; i++)
    {
        ...
        mutual_diff = secondary - primary;
        FSP_ERROR_RETURN(secondary - primary < TOUCH_MUTUAL_INT16_MAX, FSP_ERR_INVALID_HW_CONDITION);
        FSP_ERROR_RETURN(secondary - primary > -TOUCH_MUTUAL_INT16_MAX, FSP_ERR_INVALID_HW_CONDITION);
...
/*******************************************************************************************************************//**
 * @brief Setup measurement by SMS. Implements @ref touch_api_t::smsSet.
 *
 * @retval FSP_SUCCESS              Successfully closed.
 * @retval FSP_ERR_ASSERTION        Null pointer passed as a parameter.
 * @retval FSP_ERR_NOT_OPEN         Module is not open.
...
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE == 1)
    FSP_ASSERT(p_ctrl);
    FSP_ERROR_RETURN(TOUCH_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
#endif

    /* The user calls RM_TOUCH_DataGet() after returning from SMS. */
    /* At that time, to immediately touch-on the button that exceeds the threshold. */
    for (button_id = 0; button_id < p_instance_ctrl->p_touch_cfg->num_buttons; button_id++)
    {
        p_instance_ctrl->binfo.p_on_count[button_id]  = p_instance_ctrl->binfo.on_freq;
        p_instance_ctrl->binfo.p_off_count[button_id] = 0;
    }

    /* The value is requested in this format */
    count_filter = (uint16_t)(p_instance_ctrl->binfo.on_freq | (uint16_t)(p_instance_ctrl->binfo.off_freq << 8));

    err = p_instance_ctrl->p_ctsu_instance->p_api->smsSet(p_instance_ctrl->p_ctsu_instance->p_ctrl,
...
fsp_err_t RM_TOUCH_Close (touch_ctrl_t * const p_ctrl)
{
...
#if TOUCH_CFG_MONITOR_ENABLE
 #if (TOUCH_CFG_UART_MONITOR_SUPPORT == 1)
    g_touch_uart_open_bitmap &= (uint32_t) ~((uint32_t)1 << p_instance_ctrl->p_touch_cfg->number);
 ...
 #endif
 #if (TOUCH_CFG_NUM_SLIDERS != 0)
        num += g_touch_slider_index;
 #endif
 #if (TOUCH_CFG_NUM_WHEELS != 0)
        num += g_touch_wheel_index;
 #endif
        if (0 == num)
        {
            TOUCH_UART_STOP();
...
void touch_button_on (touch_button_info_t * p_binfo, uint16_t value, uint8_t button_id)
{
    *(p_binfo->p_off_count + button_id) = 0; /* non_touch count reset */
    ...
    {
        p_binfo->status |= (uint64_t) ((uint64_t)1 << button_id);
    }
    else
    {
...
void touch_button_drift (touch_button_info_t * p_binfo, uint16_t value, uint8_t button_id)
{
    if (0 != p_binfo->drift_freq)
    {
        ...
        if (0 == (p_binfo->status & ((uint64_t) ((uint64_t)1 << button_id))))
        {
            /* It is an addition for the drift correction average calculation */
                          ...
                          uint8_t slider_id)
{
    ...
    if (dsum > *(p_sinfo->p_threshold + slider_id))
    {
        ...
        /* x : y = d1 : d2 */
        d3 = (uint16_t) (TOUCH_DECIMAL_POINT_PRECISION + (uint32_t)((uint32_t)(d2 * TOUCH_DECIMAL_POINT_PRECISION) / d1));
...
void touch_uart_callback (uint16_t event)
{
    ...
    if (g_touch_uart_rx_num == TOUCH_TUNING_COMMAND_BUF_NUM)
    {
        /* Restart reception */
        switch (g_touch_uart_rx_buf[1] >> 4)
        {
            ...
            case TOUCH_UART_COMMAND_TOP_NUM:
            {
                ...
                TOUCH_UART_RECEIVE(&g_touch_uart_rx_buf[TOUCH_TUNING_COMMAND_BUF_NUM] ,g_touch_uart_rx_num);
                break;
            }
 #endif
 #if (TOUCH_CFG_UART_TUNING_SUPPORT == 1)
    ...
    if (g_touch_uart_rx_num == (TOUCH_UART_RECIEVE_BUF_SIZE - TOUCH_TUNING_COMMAND_BUF_NUM))
    {
        ...
        if (g_touch_uart_rx_buf[1] == TOUCH_UART_COMMAND_START)
        {
            ...
            if (0 == ((uint32_t) ((uint32_t)1 << num) & g_touch_uart_open_bitmap))
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    switch (g_touch_uart_rx_buf[6]) /* data type */
                    {
                        ...
                        {
  #if (BSP_FEATURE_CTSU_VERSION == 2)
                            *p_ctsusnum &= (uint32_t) (~((uint32_t)TOUCH_UART_CTSUSNUM_MASK << TOUCH_UART_CTSUSNUM_SHIFT));
                            *p_ctsusnum |= (uint32_t) ((uint32_t)write_data << TOUCH_UART_CTSUSNUM_SHIFT);
  #endif
  #if (BSP_FEATURE_CTSU_VERSION == 1)
                        ...
                        {
  #if (BSP_FEATURE_CTSU_VERSION == 2)
                            *p_ctsusdpa &= (uint32_t) (~((uint32_t)TOUCH_UART_CTSUSDPA_MASK << TOUCH_UART_CTSUSDPA_SHIFT));
                            *p_ctsusdpa |= (uint32_t) ((uint32_t)write_data << TOUCH_UART_CTSUSDPA_SHIFT);
  #endif
  #if (BSP_FEATURE_CTSU_VERSION == 1)
        ...
        /* Start transmission */
        g_touch_uart_transmit_flag = 1;
        TOUCH_UART_SEND((uint8_t *) &g_touch_monitor_buf[0], index);
        return;
    }
 #endif
    ...
    if (g_touch_uart_rx_num == (TOUCH_TUNING_RECIEVE_BUF_SIZE - TOUCH_TUNING_COMMAND_BUF_NUM))
    {
        ...
        {
            index = 5;
            ((uint8_t *)&port_adress)[0] = g_touch_uart_rx_buf[5];
            ((uint8_t *)&port_adress)[1] = g_touch_uart_rx_buf[6];
            ((uint8_t *)&port_adress)[2] = g_touch_uart_rx_buf[7];
            ((uint8_t *)&port_adress)[3] = g_touch_uart_rx_buf[8];

            write_byte = g_touch_uart_rx_buf[9];

            if (1 == write_byte)
            {
                (* ((uint8_t *) port_adress)) |= g_touch_uart_rx_buf[10];
            }
            else if (2 == write_byte)
            {
                (* port_adress) |= ((uint32_t)g_touch_uart_rx_buf[10] |
                                   ((uint32_t)g_touch_uart_rx_buf[11] << 8 ));
            }
            else
            {
                (* port_adress) |= ((uint32_t)g_touch_uart_rx_buf[10] |
                                   ((uint32_t)g_touch_uart_rx_buf[11] << 8 ) |
                                   ((uint32_t)g_touch_uart_rx_buf[12] << 16) |
                                   ((uint32_t)g_touch_uart_rx_buf[13] << 24));
            }
        }
        ...
        /* Start transmission */
        g_touch_uart_transmit_flag = 1;
        TOUCH_UART_SEND((uint8_t *) &g_touch_tuning_tx_buf[0], index);
        return;
    }
 #endif
...
void touch_tuning_qe_get_cfg (touch_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    for (i = 0; i < num_elements; i++)
    {
        if (TOUCH_TUNING_MODE_MEASURE_PHASE1 == g_touch_tuning_mode)
        {
            ...
            g_touch_tuning_element_cfgs[i].sdpa = TOUCH_CFG_PHASE1_SDPA_VALUE;
